...
The UI sends `auth:start`; the controller calls unauthenticated `POST
/v1/auth/start` and opens a short-lived, launch-keyed backend URL with
`figma.openExternal()`. The backend interstitial validates that key before
redirecting to Google. The browser performs the OAuth code exchange at the
backend callback. The UI owns a one-second poll
...
Manager supplies the OAuth secret and session pepper to Cloud Run. Auth-flow and
session records include `purgeAt` fields governed by Firestore TTL policies.
