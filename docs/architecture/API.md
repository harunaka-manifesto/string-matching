...
| GET    | `/oauth/start?flowId=&launchKey=` | Validate the browser launch key and redirect to Google.   |
...
exponential backoff and jitter. Final 429s map to `SHEET_RATE_LIMITED`; final
5xx responses map to `SHEET_READ_FAILED`. Sheet values, OAuth codes, tokens, and
source URLs are not included in logs or errors.
